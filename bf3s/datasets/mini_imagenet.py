...
class MiniImageNetBase(data.Dataset):
    ...
    def __init__(self, transform_test, transform_train, phase="train",  load_single_file_split=False,
                 file_split=None, do_not_use_random_transf=False, data_dir="./datasets/MiniImagenet"):
        print(f"==> Download MiniImageNet dataset at {data_dir}")
        train_categories_train_phase = os.path.join(data_dir, "miniImageNet_category_split_train_phase_train.pickle")
        train_categories_val_phase = os.path.join(data_dir, "miniImageNet_category_split_train_phase_val.pickle")
        train_categories_test_phase = os.path.join(data_dir, "miniImageNet_category_split_train_phase_test.pickle")
        val_categories_val_phase = os.path.join(data_dir, "miniImageNet_category_split_val.pickle")
        test_categories_test_phase = os.path.join(data_dir, "miniImageNet_category_split_test.pickle")

        self.phase = phase
        if load_single_file_split:
            assert file_split in ("category_split_train_phase_train", "category_split_train_phase_val",
                                  "category_split_train_phase_test", "category_split_val", "category_split_test")
            ...
            if self.phase == "train":
                ...
                data_train = utils.load_pickle_data(train_categories_train_phase)
                ...
                # base classes
                data_train = utils.load_pickle_data(train_categories_train_phase)
                data_val = utils.load_pickle_data(val_categories_val_phase)
                ...
                if self.phase == "test":
                    ...
                    data_base = utils.load_pickle_data(train_categories_test_phase)
                    # load data that will be use for evaluating the few-shot
                    # recogniton accuracy on the novel classes.
                    data_novel = utils.load_pickle_data(test_categories_test_phase)
                else:  # phase=='val'
                    # load data that will be used for evaluating the recognition
                    # accuracy of the base classes.
                    data_base = utils.load_pickle_data(train_categories_val_phase)
                    # load data that will be use for evaluating the few-shot
                    # recogniton accuracy on the novel classes.
                    data_novel = utils.load_pickle_data(val_categories_val_phase)
                    pass
        ...
        self.transform = self.transform_test if (self.phase == "test" or self.phase == "val"
                                                 ) or do_not_use_random_transf else self.transform_train
        pass
    ...
    def __getitem__(self, index, transform_mode=None):
        ...
        if transform_mode is None:
            ...
            img = self.transform_train(img) if transform_mode == "train" else self.transform_test(img)
            pass
        return img, label
...
class MiniImageNet(MiniImageNetBase):
    ...
    def __init__(self, phase="train", image_size=84, load_single_file_split=False,
                 file_split=None, do_not_use_random_transf=False, cutout_length=0):
        ...
        if image_size == 84:
            ...
            transform_train = transforms.Compose([transforms.RandomCrop(84, padding=8),
                                                  transforms.RandomHorizontalFlip(), lambda x: np.asarray(x),
                                                  transforms.ToTensor(), normalize])
        else:
            assert image_size > 0
            transform_test = transforms.Compose([transforms.Resize(image_size), lambda x: np.asarray(x),
                                                 transforms.ToTensor(), normalize])

            transform_train = transforms.Compose([transforms.RandomCrop(84, padding=8), transforms.Resize(image_size),
                                                  transforms.RandomHorizontalFlip(), lambda x: np.asarray(x),
                                                  transforms.ToTensor(), normalize])
            pass
        ...
        super().__init__(transform_test=transform_test, transform_train=transform_train, phase=phase,
                         load_single_file_split=load_single_file_split, file_split=file_split,
...
class MiniImageNet80x80(MiniImageNet):
    ...
    def __init__(self, phase="train", load_single_file_split=False,
                 file_split=None, do_not_use_random_transf=False, cutout_length=0,):
        super().__init__(phase=phase, image_size=80, load_single_file_split=load_single_file_split,
                         file_split=file_split, do_not_use_random_transf=do_not_use_random_transf,
...
class MiniImageNet3x3Patches(MiniImageNetBase):
    ...
    def __init__(self, phase="train", load_single_file_split=False, file_split=None,
                 do_not_use_random_transf=False, image_size=90, patch_jitter=6):
        probability = 0.66
        ...
        def crop_3x3_patches(image):
            return utils.image_to_patches(image, is_training=False, split_per_side=3, patch_jitter=patch_jitter)

        def crop_3x3_patches_random_jitter(image):
            return utils.image_to_patches(image, is_training=True, split_per_side=3, patch_jitter=patch_jitter)

        normalize = transforms.Normalize(mean=_MEAN_PIXEL, std=_STD_PIXEL)
        transform_test = transforms.Compose([transforms.Resize(image_size), lambda x: np.asarray(x),
                                             transforms.ToTensor(), normalize, crop_3x3_patches])
        transform_train = transforms.Compose([random_grayscale, transforms.RandomHorizontalFlip(),
                                              transforms.Resize(image_size), lambda x: np.asarray(x),
                                              transforms.ToTensor(), normalize, crop_3x3_patches_random_jitter])
...
class MiniImageNetImagesAnd3x3Patches(MiniImageNetBase):
    ...
    def __init__(self, phase="train", load_single_file_split=False, file_split=None, do_not_use_random_transf=False,
                 image_size=84, cutout_length=0, image_size_for_patch=96, patch_jitter=8):
        normalize = transforms.Normalize(mean=_MEAN_PIXEL, std=_STD_PIXEL)
        ...
        transform_patch_test = transforms.Compose([transforms.Resize(image_size_for_patch), lambda x: np.asarray(x),
                                                   transforms.ToTensor(), normalize, crop_3x3_patches])
        transform_patch_train = transforms.Compose([random_grayscale, transforms.RandomHorizontalFlip(),
                                                    transforms.Resize(image_size_for_patch), lambda x: np.asarray(x),
                                                    transforms.ToTensor(), normalize, crop_3x3_patches_random_jitter])
